...
function isArabicOnly(str) {
  // Regular expression that matches Arabic Unicode characters
  var arabicRegex = /[\u0600-\u06FF]/;
  // Check if the string contains only Arabic characters
...
function getParaphrasedResult() {
	...
	if (!isArabicOnly(text)) {
        error_box.innerHTML = "Sorry only Arabic is supported.";
        error_box.style.display = "block";
    }
    else {
